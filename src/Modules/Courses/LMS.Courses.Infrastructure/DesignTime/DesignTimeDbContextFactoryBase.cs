    ...
    where TContext : DbContext
{
    ...
    protected DatabaseConfiguration DatabaseConfiguration { get; set; } = null!;
    ...
    protected virtual TContext CreateContext(string[] args)
    {
        ...
        if (string.IsNullOrWhiteSpace(connectionString))
            throw new InvalidOperationException("Could not build database connection string.");
        ...
        optionsBuilder.UseNpgsql(connectionString, options =>
        {
            ...
            options.MigrationsHistoryTable("__EFMigrationsHistory", "courses");
        });
    ...
    private void InitializeConfiguration()
    {
        ...
        if (Path.GetFileName(currentDirectory).Equals("LMS.App", StringComparison.OrdinalIgnoreCase))
        {
            location = currentDirectory;
        }
        else
        {
            location = Path.Combine(currentDirectory, "src", "LMS.App");
        }
